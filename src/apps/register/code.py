...
# else the request failed, print error code, flash red lights and exit.
  ...
  err_label = Label(font=FONT, text=wrapped_err, color=WHITE)
  err_label.x = 5
  err_label.y = err_label.height // 2 + 5
  # clear_screen() installs a new root_group, so build the group after clearing
  clear_screen(LCD)
  err_group = displayio.Group()
  err_group.append(err_label)
  LCD.root_group.append(err_group)
  rsp.close()
  for i in range(3):
    NP.fill(RED)
    NP.show()
