...
def welcome_screen():
    ...
    try:
        bmp = OnDiskBitmap(SPLASH_IMAGE)
    except OSError:
        # No art on the badge: fall back to the original text screen.
        splash.append(center_text_x_plane(EPD, "It's not Simon!"))
        splash.append(
            center_text_y_plane(EPD,
            center_text_x_plane(EPD,
            wrap_message(EPD, "[ PRESS ANY BUTTON TO START ]")
        )))
    else:
        splash.append(TileGrid(bmp, pixel_shader=bmp.pixel_shader))
...
def score_screen(score):
    ...
    # A failed upload should not end the game. post_to_leaderboard() also draws
    # its own prompt/status on the LCD and never puts back what was there, so
    # stash and restore it (try/finally, since the failure case matters most).
    lcd_before = LCD.root_group
    try:
        post_to_leaderboard(score)
    except Exception as e:
        print("simon: leaderboard post failed:", e)
    finally:
        LCD.root_group = lcd_before
