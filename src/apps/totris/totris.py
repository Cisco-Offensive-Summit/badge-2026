...
class Totris:
    ...
    def single_player_start(self):
        ...
        # The bitmap carries its own palette (index 0 black, index 1 white), so
        # it is drawn as authored -- no inversion needed.
    ...
    def single_player_game(self):
        while self.single_player_start():
            ...
            time.sleep(3)
            # A failed upload should not end the game. The LCD is not restored
            # here on purpose: single_player_start() repaints it on the next
            # pass through the loop.
            try:
                post_to_leaderboard(self.game_score)
            except Exception as e:
                print("totris: leaderboard post failed:", e)
