...
SPLASH_IMAGE = '/apps/masher/splash.bmp'


def epd_splash():
    """Put the MASHER title card on the EPD at startup.

    Silently does nothing if the bitmap is missing so a stripped-down install
    still plays.
    """
    try:
...
def score_screen(elapsed):
    ...
    # post_to_leaderboard() draws its own prompt/status onto the LCD and does
    # not put back what was there, so stash the caller's screen and restore it.
    # try/finally so a failed post still leaves the LCD in a sane state.
