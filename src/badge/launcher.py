...
class LauncherUI:
    ...
    def lcd_change_app(self, app: APP):
        ...
        self.scroll_label_group.full_text = scroll_text
        self.scroll_label_group.current_index = 0
        # re-render straight away so the new name appears windowed rather than
        # as the full string until the next animation tick
    ...
    async def lcd_animate_label(self):
        while True:
            if self.hold_label < 5:
                ...
                # update() advances the scroll window; update(force=True) only
                # re-renders the current window without advancing it, which is
                # why forcing here looked like a flicker instead of scrolling.
                self.scroll_label_group.update()
