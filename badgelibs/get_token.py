...
PROVISIONING_SECRET_PATH = '/provisioning_secret.txt'
PROVISIONING_SECRET_HEADER = 'X-Badge-Provisioning-Secret'
JSON = {
...
def read_provisioning_secret():
  try:
    with open(PROVISIONING_SECRET_PATH, 'r') as fp:
...
def delete_provisioning_secret():
  try:
    os.remove(PROVISIONING_SECRET_PATH)
    return True
  except OSError:
    flash_red(5)
    return False
...
def get_token():
  ...
  provisioning_secret = read_provisioning_secret()
  if not provisioning_secret:
    return False

  headers = HEADERS.copy()
  headers[PROVISIONING_SECRET_HEADER] = provisioning_secret
  ...
  if rsp.status_code == 200:
    ...
    try:
      with open("/secrets.py", "a") as fp:
        ...
        fp.flush()
      return delete_provisioning_secret()
